function save_result(savefilename, savedir, savemode, U, xx, varformat, append_flag, compute_temp, temp_mode, T,...
	...
	if append_flag == 0
		...
		keyset = {'xend', 'ncells', 'tend', 'tsteps', 'c1', 'c2', 'gam1', 'gam2', 'pinf1', 'pinf2', 'updatemesh', 'bc', 'v_bc', 'compute_temp', 'temp_mode', 'opt_tol', 'cv1', 'muacc', 'Ls', 'n', 'as0', 'Ri0', 'avg_alpha', 'std_alpha', 'distribution', 'savemode'};
		keytypes={ '%5f', '%d', '%g', '%d', '%d', '%d', '%4.2f', '%4.2f', '%g', '%2.1f', '%d', '%d', '%g' ,'%d', '%d', '%g', '%5.2f', '%4g', '%f', '%d', '%f', '%g', '%4.2f', '%4.2f', '%d', '%d'};
		valueset = [xend, ncells, tend, tsteps, c1, c2, gam1, gam2, pinf1, pinf2, updatemesh, bc, vbc, compute_temp, temp_mode, opt_tol, cv1, muacc, Ls, n, as0, Ri0, avg_alpha, std_alpha, distribution, savemode];
		optstr = '';
		typestr = ''; 
		varstr1 = '';
		vartype1 = '';
